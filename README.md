# Kittygram - социальная сеть для размещение фотографий котиков.
...
- В директории _/etc/systemd/system/_ создайте файл _gunicorn_kittygram.service_ `sudo nano /etc/systemd/system/gunicorn_kittygram.service`  со следующим кодом (без комментариев):
